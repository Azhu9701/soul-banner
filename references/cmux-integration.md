...
## 架构（v3: tail -f 显示器 — 零 token）

```
主 agent（纯协作者）              cmux Workspace              魂 Agent
┌────────────────────┐       ┌─────────────────────┐     ┌──────────────┐
│ 1. mkdir + touch   │       │ pane 1: tail -f 费曼 │     │ 费曼 agent   │
│ 2. cmux_new_workspace      │  "■ 分析中..."      │     │ Write → 文件 │
│    + cmux_new_split │─────▶│                     │     └──────────────┘
│ 3. spawn Agent("费曼")─────│─────── 写入 ────────▶│    ┌──────────────┐
│ 4. spawn Agent("鲁迅")─────│─────── 写入 ────────▶│    │ 鲁迅 agent   │
│ 5. 监控文件增长     │◀─────│ pane 2: tail -f 鲁迅 │     │ Write → 文件 │
│ 6. Read 文件        │      │  "■ 分析中..."      │     └──────────────┘
│ 7. spawn 辩证综合官  │      └─────────────────────┘
└────────────────────┘
```
...
| cmux 显示 | `tail -f` 终端 | **实时可见** | **0** |
| 魂分析 | `Agent(subagent_type="{魂名}")`（主 agent 直接 spawn） | 不可见 | 正常 |
| 辩证综合 | `Agent(subagent_type="辩证综合官")` | 不可见 | 正常 |

**核心**：cmux 只提供可视化布局（终端 pane + tail -f），不参与任何分析和调度。与传统模式的区别仅在于多了一个零成本的显示面板。
