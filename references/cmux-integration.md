...
```
主 agent（纯协作者）                  cmux Workspace
┌──────────────────┐               ┌─────────────────────────┐
│ 1. match.py      │               │ pane 1: 毛泽东 🟡       │
│ 2. spawn 幡主审查  │  cmux MCP    │  "从矛盾论角度分析..."    │
│ 3. cmux-plan.py  │──────────────▶│                         │
│ 4. 分发任务       │               │ pane 2: 费曼 🟡         │
│ 5. 监控进度       │◀──────────────│  "让我先检查假设..."      │
│ 6. 收集输出→写文件 │  cmux_read   │                         │
│ 7. spawn 辩证综合官│               │ pane 3: 鲁迅 🟡         │
│ 8. 落盘          │               │  "这问题的根柢在于..."    │
└──────────────────┘               └─────────────────────────┘
```
...
| 层 | 工具 | 可视化 | 适用 |
|----|------|--------|------|
| 匹配审查 | `Agent(subagent_type="列宁")` | 不可见 | 快速匹配判断，不占 pane |
| 魂执行 | `cmux_launch_agents(cli="claude")` | **实时可见** | 魂的思考过程 |
