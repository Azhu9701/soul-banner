...
- **收魂** → **炼化** → **审查** → 入幡
- 任务来临时 **附体**（单魂/合议/辩论/接力/学习）→ **幡主审查匹配**（基于主义主义目录 compat/incompat 标注）→ 子 agent 并行 → **辩证综合** → 反馈闭环 → **实践开口检测** → Obsidian 存档
...
| `炼化 {人物名}` | raw 素材炼化为 Soul Profile，评估 |
| `幡中有什么魂` | 查看魂魄列表及分布 |
...
├── soul-profile-format.md        # 体系 + 审查委员会制度（按需加载）
...
魂间互审（入幡魂）：互相审查，报告保存至 `reviews/`。轮值制同样适用于定期互审调度。
...
11. **实践开口检测**（v3.5 新增）：辩证综合完成后，主 agent 必须执行：

    ```
    ## 实践开口检测（强制环节）
    
    辩证综合官必须回答：
    ① 本次分析是否存在现有魂的方法论无法覆盖的盲区？[是/否]
    ② 若存在——这些盲区是"可收魂填充的经典位"还是"必须从实践中获取的新经验"？
    ③ 若为后者——生成一个实践议题，包含：
    ...
    **原则**：系统不试图填实践盲区——输出实践议题。使用者带着议题去实践，带回来的新事实更新魂的 profile 或触发新魂炼化。这是从「过去知识的交通系统」转向「实践-理论的反馈循环」的关键步骤。
...
- **幡主**：由用户指定一个入幡魂担任，负责每次附体的匹配审查。每次附体必须 spawn 幡主子 agent 独立审查匹配。
- **第二审查官**：由用户指定另一个入幡魂担任。触发条件：幡主审查推荐幡主自己为分析魂时——审查者与分析者为同一人，需独立第二意见。
...
魂的功能由 ismism 四维坐标唯一确定。不再使用额外标签体系。详见 **[soul-profile-format.md](soul-profile-format.md)**。
...
- **[soul-profile-format.md](soul-profile-format.md)** — 体系 + 审查委员会六维度否决制 + 轮值细则
