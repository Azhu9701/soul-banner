...
自 2026-05-01 起，审查委员会（列宁、毛泽东、邓小平）轮值执行新魂入幡审查。轮值表、规则及金魂互审调度详见 **[soul-profile-format.md](soul-profile-format.md)#审查轮值制**。
...
每次附体是 6 步仪轨：匹配魂 → 幡主审查 → 执行附体 → **humanizer 去 AI 痕迹（魂输出后处理）** → 更新 registry + 存档 Obsidian（后两步并行）。Task 模板详见 `CLAUDE.md`。

完整流程、魂选择原则、匹配公式、衔接点审查细节见 **[auto-possess.md](auto-possess.md)**。
